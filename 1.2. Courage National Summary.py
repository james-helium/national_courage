...
# Read the data
filtered_full_sample = pd.read_csv("data/filtered_full_sample.csv").drop(
    "Unnamed: 0", axis=1
)
un_populations = pd.read_csv("data/un_populations.csv")
# process UN reference data

un_populations = un_populations[
...
pop_25_49 = un_populations[
    un_populations.AgeGrp.isin(["25-29", "30-34", "35-39", "40-44", "45-49"])
]
pop_50_69 = un_populations[
    un_populations.AgeGrp.isin(["50-54", "55-59", "60-64", "65-69"])
]
pop_70_99 = un_populations[
    un_populations.AgeGrp.isin(["70-74", "75-79", "80-84", "85-89", "90-94", "95-99"])
]
...
un_populations = pd.concat(
    [pop_00_14, pop_15_19, pop_20_24, pop_25_49, pop_50_69, pop_70_99]
)

# get total population
pop_total = (
    un_populations.groupby("Location").sum().reset_index()[["Location", "PopTotal"]]
)
# ratio of population in each age-sex group
un_populations = un_populations.merge(pop_total, on="Location", how="left")
un_populations.rename(
    columns={"PopTotal_x": "AgeTotal", "PopTotal_y": "PopTotal"}, inplace=True
)
...
un_populations.loc[
    un_populations["Location"] == "China, Hong Kong SAR", "Location"
] = "Hong Kong"
un_populations.loc[
    un_populations["Location"] == "China, Taiwan Province of China", "Location"
] = "Taiwan"
un_populations.loc[
    un_populations["Location"] == "Iran (Islamic Republic of)", "Location"
] = "Iran"
un_populations.loc[
    un_populations["Location"] == "Republic of Korea", "Location"
] = "South Korea"
un_populations.loc[
    un_populations["Location"] == "Russian Federation", "Location"
] = "Russia"
un_populations.loc[un_populations["Location"] == "Türkiye", "Location"] = "Turkey"
un_populations.loc[
    un_populations["Location"]
    == "United Kingdom of Great Britain and Northern Ireland",
    "Location",
] = "United Kingdom"
un_populations.loc[
    un_populations["Location"] == "United States of America", "Location"
] = "United States"
un_populations.loc[
    un_populations["Location"] == "Venezuela (Bolivarian Republic of)", "Location"
] = "Venezuela"
...
filtered_full_sample["demo_age"] = pd.cut(
    ...
    labels=["0-14", "15-19", "20-24", "25-49", "50-69", "70-99"],
)

# summarise magic_COUR as mean, group by geo_country, demo_gender, demo_age
courage_summary = (
    filtered_full_sample.groupby(["geo_country", "demo_gender", "demo_age"])
    .mean()
    .reset_index()
)
...
courage_summary = courage_summary[
    ["geo_country", "demo_age", "magic_COUR_x", "magic_COUR_y"]
]
...
courage_summary["weighted_courage"] = (
    courage_summary.CourFemale + courage_summary.CourMale
)
courage_summary["unweighted_courage"] = (
    filtered_full_sample[["geo_country", "magic_COUR"]]
    .groupby("geo_country")
    .mean()
    .reset_index()["magic_COUR"]
)
# save to csv
courage_summary = courage_summary[
    ["Location", "weighted_courage", "unweighted_courage"]
]
courage_summary.columns = ["Country", "Weighted Average", "Unweighted Average"]
