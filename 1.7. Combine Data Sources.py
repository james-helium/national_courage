...
national_courage = national_courage.merge(country_names, left_on="Country", right_on="Country Name", how="left")
...
national_courage.loc[national_courage["Country"] == "South Korea", "Country Code"] = "KOR"
...
# rename columns
national_courage.columns = ['national_courage']
# standardise ipums controls with country names and code
ipums_controls = ipums_controls.merge(country_names, left_on="Country", right_on="Country Name", how="left")
...
wvs_idv = wvs_idv.merge(country_names, left_on="Country", right_on="Country Code", how="left")
...
all_national_data = pd.concat(
    [main_controls, ipums_controls, ipums_idv, wvs_idv, national_courage],
    axis=1
)
# add country names back to the data frame
all_national_data = all_national_data.merge(country_names, left_index=True, right_on="Country Code", how="left")
# drop column index
all_national_data = all_national_data.drop("index", axis=1)

# only keep the countries that are in national_courage
all_national_data = all_national_data[all_national_data["Country Code"].isin(national_courage.index.get_level_values(0))]
