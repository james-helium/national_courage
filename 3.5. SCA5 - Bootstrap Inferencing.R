...
get_bootstrap_results <- function(Specs, R = 100){
  ...
  for(row in 1:nrow(bootstrap_beta)){
    ...
    for(col in 2:ncol(random_null)){
      ...
      } else{this_spec <- data.frame(this_spec, control_education = rep(0, each = 80))}
      ...
      } else{this_spec <- data.frame(this_spec, control_economics = rep(0, each = 80))}
      ...
      } else{this_spec <- data.frame(this_spec, control_urbanised = rep(0, each = 80))}
      ...
      for(i in 1:ncol(this_spec)){
        ...
        if(all(this_spec[,i] == 0) == FALSE){ this_spec[,i] <- scale(this_spec[,i])}
      }
      ...
      if(!is.na(bootstrap_sigV[row, k]) & bootstrap_sigV[row, k] < 0.05){bootstrap_sigV[row, k] <- 1} 
      else{bootstrap_sigV[row, k] <- 0}
      #print for progress
      #var name if for later for-loop usage, can be disregarded
      print(paste('Boostrapping', row, 'out of', R, 
