...
sca_panel <- function(Specifications, variable_name) {
  ...
  for (row in 1:nrow(Specifications)) {
    ...
    Var.Name <- c(
      Var.Name, Specifications$y[row], Specifications$x[row],
...
pdf(
  file = "results/sca_plots/figure_for_globe_practices.pdf",
  width = 12, height = 13.5
)
for (i in 1:length(GLOBE_practices_code)) {
  practice_code <- GLOBE_practices_code[i]
  practice_name <- GLOBE_practices_name[i]
  Specs_GLOBE <- read.csv(
    paste0("results/raw_specification_results/Specs_", practice_code, ".csv")
  )[, -1]
  plot(sca_combine(Specs_GLOBE, practice_name))
}
dev.off()
